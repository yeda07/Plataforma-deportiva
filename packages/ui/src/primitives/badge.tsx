...
const toneClassName: Record<BadgeTone, string> = {
  ...
  muted: "bg-surface-subtle text-muted-foreground ring-border"
};
...
export function Badge({ children, className, tone = "muted", ...props }: BadgeProps) {
  return (
    <span
      className={cn(
        "inline-flex min-h-6 items-center rounded-md px-2.5 text-caption font-bold ring-1 transition duration-200",
        toneClassName[tone],
        className
      )}
      {...props}
