  ...
  return (
    <label className="grid gap-2 text-label text-foreground">
      ...
      <span className="relative block">
        <span
          aria-hidden="true"
          className="absolute left-3 top-1/2 -translate-y-1/2 text-label text-primary"
        >
          /
        </span>
        <input
          className={cn(
            "min-h-11 w-full rounded-md border border-input bg-surface-elevated px-9 text-body text-foreground shadow-xs outline-none transition duration-200 placeholder:text-muted-foreground hover:border-primary/50 hover:bg-card-hover focus-visible:border-primary focus-visible:ring-2 focus-visible:ring-ring disabled:cursor-not-allowed disabled:opacity-55",
            className
          )}
          type="search"
          {...props}
